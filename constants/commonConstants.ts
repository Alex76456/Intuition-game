...
export const gameConfig = {
  ...
  GET_RESULT_MESSAGE: ({ winningNumber, winningMessage }: { winningMessage: MessageType; winningNumber: number }) =>
    getResultMessage({ winningNumber, winningMessage }),
  GET_REMAINING_SECONDS: ({ nextResultDate }: { nextResultDate: number }) => getRemainingSeconds({ nextResultDate }),
}
